...
export function VoiceSlider() {
  ...
  const go = useCallback((next: number) => {
    setIndex((next + cultureVoices.length) % cultureVoices.length);
  }, []);

  useEffect(() => {
    if (paused) return;
    const id = window.setInterval(() => {
      setIndex((current) => (current + 1) % cultureVoices.length);
    }, INTERVAL_MS);
    ...
    >
      <div className="grid lg:grid-cols-2">
        <div className="relative min-h-[280px] sm:min-h-[420px]">
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-[#0c0d10] via-transparent to-black/20 lg:bg-gradient-to-r lg:from-transparent lg:to-[#0c0d10]" />
        </div>
        <div
          className="relative z-10 flex flex-col justify-between p-8 sm:p-12"
          onMouseEnter={() => setPaused(true)}
          onMouseLeave={() => setPaused(false)}
        >
          <div>
            <p className="text-[11px] tracking-[0.32em] text-[#c4a05a] uppercase">{voice.kicker}</p>
            <blockquote className="font-heading mt-6 text-2xl leading-snug text-zinc-50 sm:text-3xl">
              “{voice.quote}”
            </blockquote>
          ...
          <div className="mt-10 flex items-center justify-between gap-4">
            ...
            <div className="flex gap-2">
              <button
                ...
                onClick={() => setIndex((current) => (current - 1 + cultureVoices.length) % cultureVoices.length)}
              ...
              <button
                ...
                onClick={() => setIndex((current) => (current + 1) % cultureVoices.length)}
                className="inline-flex size-11 items-center justify-center rounded-full border border-white/15 text-zinc-200 transition hover:border-[#c4a05a]/50 hover:text-[#c4a05a]"
              >
